...
## Manual Merge Helper
Need to move the project into another repository or assignment template? The
fastest way is to dump each tracked file directly from the shell and paste it
into the destination editor. From the repo root run:
...
cat README.md
cat main.py
cat src/portfolio_pipeline.py
```

Each command prints the full contents of the corresponding file so you can copy
and paste without worrying about hidden formatting. If you prefer a single file
that concatenates everything for archival, you can also run:

```bash
python - <<'PY'
from pathlib import Path
root = Path('.')
for path in [Path('.gitignore'), Path('requirements.txt'), Path('README.md'), Path('main.py'), Path('src/portfolio_pipeline.py')]:
    print(f"\n# >>> {path}")
    print(path.read_text())
PY
```

The snippet prints each file preceded by a header so you can distinguish which
segment belongs to which path when copying the combined output into another
document (e.g., a learning management system submission box).
